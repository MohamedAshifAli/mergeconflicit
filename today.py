...
from keras.layers import BatchNormalization, Dropout, Flatten, Dense, Activation, MaxPooling2D, Conv2D, LSTM, \
    GlobalAveragePooling2D, MaxPool2D, multiply, Add, GlobalMaxPooling2D, Reshape, Lambda, SimpleRNN
...
def DNN_Model(X_train, y_train, X_test, y_test, epochs):
    ...
    ann.fit(X_train, y_train, epochs=epochs, batch_size=64, verbose=1)
    print(colored("[INFO] -------Testing DNN---------", "magenta"))
    YPred = np.argmax(ann.predict(X_test),axis=1)
    return YPred
...
def TP_Analysis(features,labels):
    ...
    for p in range(len(tr)):
        ...
        Y_pred1, Y_true1 = RNN(xtrain, y1train, xtest, y1test, epochs[1])
        Y_pred2, Y_true2 = Deep_CNN(xtrain, y1train, xtest, y1test, epochs[0])
        Y_pred3, Y_true3 = Deep_CNN(xtrain, y1train, xtest, y1test, epochs[1])
        # Y_pred4, Y_true4 = Deep_CNN(xtrain, y1train, xtest, y1test, epochs[3])
    ...
    for w in range(len(kr)):
        ...
        for k, (train, test) in enumerate(kfold):
            if k==0:
                ...
                Y_pred1, Y_true1 = Deep_CNN(xtrain, y1train, xtest, y1test, epochs[0])
                Y_pred2, Y_true2 = Deep_CNN(xtrain, y1train, xtest, y1test, epochs[1], options[1])
                Y_pred3, Y_true3 = Deep_CNN(xtrain, y1train, xtest, y1test, epochs[2], options[1])
                Y_pred4, Y_true4 = Deep_CNN(xtrain, y1train, xtest, y1test, epochs[3], options[1])
                Y_pred5, Y_true5 = Deep_CNN(xtrain, y1train, xtest, y1test, epochs[4], options[1])
            ...
            [ACC_3, SEN_3, SPE_3, PRE_3, REC_3, FSC_3, CSI_3, FPR_3, FNR_3, MCC_3, PPV_3, NPV_3] = parameter(acc3, sen3,
                                                                                                             spe3, pre3,
                                                                                                             rec3, fsc3,
                                                                                                             csi3, fpr3,
                                                                                                             fnr3, mcc3,
                                                                                                             ppv3, npv3)
            [ACC_4, SEN_4, SPE_4, PRE_4, REC_4, FSC_4, CSI_4, FPR_4, FNR_4, MCC_4, PPV_4, NPV_4] = parameter(acc4, sen4,
                                                                                                             spe4, pre4,
                                                                                                             rec4, fsc4,
                                                                                                             csi4, fpr4,
                                                                                                             fnr4, mcc4,
                                                                                                             ppv4, npv4)
            [ACC_5, SEN_5, SPE_5, PRE_5, REC_5, FSC_5, CSI_5, FPR_5, FNR_5, MCC_5, PPV_5, NPV_5] = parameter(acc5, sen5,
                                                                                                             spe5, pre5,
                                                                                                             rec5, fsc5,
                                                                                                             csi5, fpr5,
                                                                                                             fnr5, mcc5,
